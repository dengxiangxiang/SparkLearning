...
//        "order by count desc")
    ...
    val relationalGroupedDS: RelationalGroupedDataset = resultDS.groupBy("userId", "applicationId", "credentialType", "credentialKey", "deviceId")
    val frame: DataFrame = relationalGroupedDS.count()
    frame.printSchema()

    val rows: Array[Row] = frame.sort(col("count").desc).take(3)
